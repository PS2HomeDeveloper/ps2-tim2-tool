...
║   Formats : 4-bit | 8-bit | 16-bit | 24-bit | 32-bit               ║
╚══════════════════════════════════════════════════════════════════════╝
...
# ══════════════════════════════════════════════════════════════════════════════
#  --diff  —  Compare original TIM2 vs modified TIM2
# ══════════════════════════════════════════════════════════════════════════════

def _read_tm2_header(path: Path) -> dict:
    """
    Read TIM2 header fields needed for diff comparison.
    Returns a dict with all relevant fields, or raises on invalid file.
    """
    data = path.read_bytes()
    if data[:4] != b'TIM2':
        raise ValueError(f"Not a valid TIM2 file: {path.name}")
    ...
    pixel_data = data[img_start : img_start + img_size]
...
def _has_transparency(h: dict) -> bool:
    """
    Check if a TIM2 file has meaningful transparency.
    32-bit: any alpha < 0x80 (PS2 full alpha).
    16-bit: any pixel with A-bit = 0.
    Indexed: any CLUT entry with alpha < 0x80.
    """
    data      = h['raw']
    off       = 16
    hdr_size  = h['hdr_size']
    img_start = off + hdr_size
    img_type  = h['img_type']
    clut_size = h['clut_size']
    pix       = h['pixel_data']
    clut      = data[img_start + h['img_size'] : img_start + h['img_size'] + clut_size]

    if img_type == IMG_RGBA32:
        for i in range(3, len(pix), 4):
            if pix[i] < 0x80:
                return True
        return False
        ...
        for i in range(0, len(pix) - 1, 2):
            val = struct.unpack_from('<H', pix, i)[0]
            if (val >> 15) == 0:
                return True
        return False

    elif img_type in (IMG_INDEXED8, IMG_INDEXED4, IMG_INDEXED4_SONY):
        clut_colors = h['clut_colors']
        bytes_per   = clut_size // max(clut_colors, 1) if clut_colors > 0 else 4
        if bytes_per == 4:
...
def _count_pixel_diff(h_orig: dict, h_mod: dict) -> tuple:
    """
    Compare pixel data byte by byte.
    Returns (changed_pixels, total_pixels, max_diff).
    """
    p1 = h_orig['pixel_data']
    p2 = h_mod['pixel_data']
    img_type     = h_orig['img_type']
    total_pixels = h_orig['width'] * h_orig['height']
    changed      = 0
    max_diff     = 0

    if img_type == IMG_RGBA32:
        for i in range(0, min(len(p1), len(p2), total_pixels * 4), 4):
            d = max(abs(p1[i]-p2[i]), abs(p1[i+1]-p2[i+1]),
                    abs(p1[i+2]-p2[i+2]), abs(p1[i+3]-p2[i+3]))
            if d > 0: changed += 1
            max_diff = max(max_diff, d)

    elif img_type == IMG_RGBA16:
        for i in range(0, min(len(p1), len(p2), total_pixels * 2), 2):
            v1 = struct.unpack_from('<H', p1, i)[0] if i+1 < len(p1) else 0
            v2 = struct.unpack_from('<H', p2, i)[0] if i+1 < len(p2) else 0
            if v1 != v2: changed += 1
            max_diff = max(max_diff, abs(v1 - v2))

    else:
        for i in range(min(len(p1), len(p2), total_pixels)):
            d = abs(p1[i] - p2[i])
            if d > 0: changed += 1
            max_diff = max(max_diff, d)

    return changed, total_pixels, max_diff


def read_diff_list(list_path: Path) -> list:
    """
    Read a text file containing pairs of original/modified TIM2 files.
    ┌──────────────────────────────────────────────────────────────────┐
    │  File format:                                                    │
    │  Line 1: header (ignored) — "original  modified"                │
    │  Each subsequent line: original_path  modified_path             │
    │                                                                  │
    │  Example:                                                        │
    │      original              modified                              │
    │      hero.tm2              hero_edit.tm2                         │
    │      /orig/bg.tm2          /mod/bg.tm2                           │
    │      /a/icon.tm2           /b/icon_new.tm2                       │
    │                                                                  │
    │  Rules:                                                          │
    │  • First non-empty non-comment line = header, skipped           │
    │  • Blank lines and # comments are ignored                        │
    │  • Each data line must have exactly 2 whitespace-separated paths │
    └──────────────────────────────────────────────────────────────────┘
    """
    if not list_path.exists():
        raise FileNotFoundError(f"Diff list file not found: {list_path}")
    ...
    with open(list_path, 'r', encoding='utf-8') as f:
        for lineno, raw in enumerate(f, 1):
            ...
            if not header_skipped:
                # First non-empty line is the header row — skip it
                header_skipped = True
                continue
            parts = line.split()
            if len(parts) < 2:
                raise ValueError(
                    f"Line {lineno}: expected 2 paths, got 1.  Line: '{line}'"
                )
...
def diff_files(orig_path: Path, mod_path: Path,
               show_header: bool = True) -> dict:
    """
    Compare one original TIM2 file against one modified TIM2 file.
    Prints a detailed report and returns a result dict.
    """
    sep = '─' * 56
    ...
    if show_header:
        ...
        print(f"  Modified : {mod_path.name}")
        print(sep)
    ...
    # ─── Read both headers ────────────────────────────────────────────────────
    try:
        h_orig = _read_tm2_header(orig_path)
    except Exception as e:
        print(f"  [FAIL]  Cannot read original: {e}")
        return {'compatible': 'UNSAFE', 'issues': 1, 'warnings': 0}

    try:
        h_mod = _read_tm2_header(mod_path)
    except Exception as e:
        print(f"  [FAIL]  Cannot read modified: {e}")
        return {'compatible': 'UNSAFE', 'issues': 1, 'warnings': 0}

    # ─── 1. Format ───────────────────────────────────────────────────────────
    orig_fmt = _IMG_TYPE_NAME.get(h_orig['img_type'], f"0x{h_orig['img_type']:02X}")
    mod_fmt  = _IMG_TYPE_NAME.get(h_mod['img_type'],  f"0x{h_mod['img_type']:02X}")
    if h_orig['img_type'] == h_mod['img_type']:
        print(f"  [OK]    Format      : {orig_fmt}")
    else:
        print(f"  [FAIL]  Format      : {orig_fmt}  →  {mod_fmt}")
        issues.append('format changed')

    # ─── 2. Dimensions ───────────────────────────────────────────────────────
    ow, oh = h_orig['width'], h_orig['height']
    mw, mh = h_mod['width'],  h_mod['height']
    if ow == mw and oh == mh:
        print(f"  [OK]    Dimensions  : {ow}x{oh}")
    else:
        print(f"  [FAIL]  Dimensions  : {ow}x{oh}  →  {mw}x{mh}")
        issues.append('dimensions changed')

    # ─── 3. File size ────────────────────────────────────────────────────────
    os_ = h_orig['file_size']
    ms  = h_mod['file_size']
    if os_ == ms:
        print(f"  [OK]    File size   : {os_/1024:.2f} KB")
    else:
        sign = '+' if ms > os_ else ''
        print(f"  [WARN]  File size   : {os_/1024:.2f} KB  →  {ms/1024:.2f} KB  "
              f"({sign}{(ms-os_)/1024:.2f} KB)")
        warnings.append('file size changed')

    # ─── 4. Transparency ─────────────────────────────────────────────────────
    orig_alpha = _has_transparency(h_orig)
    mod_alpha  = _has_transparency(h_mod)
    if orig_alpha == mod_alpha:
        state = 'has transparency' if orig_alpha else 'no transparency (solid)'
        print(f"  [OK]    Transparency: {state}")
    elif orig_alpha and not mod_alpha:
        print(f"  [WARN]  Transparency: original has alpha channel,")
        print(f"                        modified appears solid (possible black background)")
        warnings.append('transparency lost')
    else:
        print(f"  [INFO]  Transparency: modified has alpha, original did not")
        infos.append('transparency added')

    # ─── 5. Pixel diff ───────────────────────────────────────────────────────
    if h_orig['img_type'] == h_mod['img_type'] and ow == mw and oh == mh:
        changed, total, max_diff = _count_pixel_diff(h_orig, h_mod)
        pct = changed / total * 100 if total > 0 else 0
        print(f"  [INFO]  Pixels changed : {changed:,} / {total:,}  ({pct:.1f}%)")
        print(f"  [INFO]  Max color diff : {max_diff} / 255")
    else:
        print(f"  [INFO]  Pixel diff skipped (format or dimensions differ)")

    # ─── Compatibility verdict ────────────────────────────────────────────────
    print(sep)
    if not issues and not warnings:
        compat = 'SAFE'
        print(f"  Compatibility : SAFE  — safe to inject back into the game")
    elif not issues and warnings:
        compat = 'WARNING'
        print(f"  Compatibility : WARNING  — likely works but review warnings above")
    else:
        compat = 'UNSAFE'
        print(f"  Compatibility : UNSAFE  — format or dimensions changed, game may crash")
    print(sep)

    return {'compatible': compat, 'issues': len(issues),
            'warnings': len(warnings), 'name': orig_path.name}


def diff_folders(orig_dir: Path, mod_dir: Path) -> None:
    """
    Compare all TIM2 files in two folders by matching filenames.
    Reports missing, extra, and per-file diff results.
    """
    sep2 = '═' * 56

    orig_files = {f.name: f for f in orig_dir.rglob('*')
                  if f.suffix.lower() in ('.tm2', '.tim2')}
    mod_files  = {f.name: f for f in mod_dir.rglob('*')
                  if f.suffix.lower() in ('.tm2', '.tim2')}

    print(f"\n{sep2}")
    print(f"  Comparing folders:")
    print(f"  Original : {orig_dir}  ({len(orig_files)} TIM2 files)")
    print(f"  Modified : {mod_dir}  ({len(mod_files)} TIM2 files)")
    print(sep2)

    # ─── Missing in modified ──────────────────────────────────────────────────
    for name in sorted(set(orig_files) - set(mod_files)):
        print(f"  [WARN]  Missing in modified  : {name}")

    # ─── Extra in modified ────────────────────────────────────────────────────
    for name in sorted(set(mod_files) - set(orig_files)):
        print(f"  [INFO]  Extra in modified (not in original) : {name}")

    # ─── Compare matching files ───────────────────────────────────────────────
    results = []
    for name in sorted(set(orig_files) & set(mod_files)):
        r = diff_files(orig_files[name], mod_files[name], show_header=True)
        results.append(r)
    ...
    print(f"\n{sep2}")
    print(f"  Summary:")
    print(f"  Compared  : {len(results)} files")
    print(f"  SAFE      : {safe}")
    print(f"  WARNING   : {warn}")
    print(f"  UNSAFE    : {unsafe}")
    print(f"  Missing   : {len(set(orig_files) - set(mod_files))}")
    print(f"  Extra     : {len(set(mod_files) - set(orig_files))}")
    print(sep2)
...
def convert_24bit(img: Image.Image, premult: bool = False,
                  use_swizzle: bool = False, use_mipmaps: bool = False,
                  **_) -> bytes:
    """
    24-bit RGB888  —  full color, no Alpha channel.
    ┌──────────────────────────────────────────────────────────┐
    │  Best for: backgrounds and textures with no transparency │
    │  3 bytes per pixel: R G B                                │
    │  Alpha is not stored — all pixels are fully opaque       │
    └──────────────────────────────────────────────────────────┘
    """
    img = img.convert('RGB')
    w, h = img.size
    ...
    if use_mipmaps:
        mip_imgs   = generate_mipmaps(img)[1:]
        mip_levels = [mip.convert('RGB').tobytes() for mip in mip_imgs]

    return build_picture_block(
        raw, b'', w, h,
        24, IMG_RGB24, CLUT_NONE, 0,
        GS_PSM_CT32, GS_CPSM_CT32,
        use_swizzle=False, mip_levels=mip_levels)
...
FORMATS = {
    ...
    '24bit': (convert_24bit, 'RGB888    | full color, no alpha       | backgrounds'),
...
def verify_tm2(path: Path) -> bool:
    ...
    # ─── 5. Image Type ───────────────────────────────────────────────────────
    valid_types = {IMG_RGBA32, IMG_RGBA16, IMG_RGB24, IMG_INDEXED8, IMG_INDEXED4, IMG_INDEXED4_SONY}
...
def extract_tm2(src_path: Path, out_ext: str) -> str:
    ...
    elif img_type == IMG_RGB24:
        # ── 24-bit RGB888 — no Alpha ──────────────────────────────────────────
        # 3 bytes per pixel: R G B — Alpha set to 255 (fully opaque)
        pixels = []
        for i in range(0, len(img_data) - 2, 3):
            r = img_data[i]
            g = img_data[i+1]
            b = img_data[i+2]
...
def list_formats():
    ...
    print("  --diff                  Compare original vs modified TIM2 files")
    print("    --original PATH ...   One or more original TIM2 files, or one folder")
    print("    --modified PATH ...   One or more modified TIM2 files, or one folder")
    print("    --diff-list FILE.TXT  Text file with pairs (original  modified)")
...
Examples:
  ...
  {PROG}                --list-formats
    ...
    parser.add_argument('--format', '-f',  choices=list(FORMATS.keys()),
                        help='Output format: 4bit | 8bit | 16bit | 24bit | 32bit')
    ...
    parser.add_argument('--diff',          action='store_true',
                        help='Compare original vs modified TIM2 files')
    parser.add_argument('--original',      metavar='PATH', nargs='+',
                        help='One or more original TIM2 files, or one folder')
    parser.add_argument('--modified',      metavar='PATH', nargs='+',
                        help='One or more modified TIM2 files, or one folder')
    parser.add_argument('--diff-list',     metavar='FILE.TXT',
                        help='Text file with pairs: original  modified  (one pair per line)')
    ...
    if args.diff or args.diff_list:
        ...
        if args.original and args.modified:
            ...
            if len(orig_list) != len(mod_list):
                ...
                sys.exit(1)
            pairs += list(zip(
                [Path(p) for p in orig_list],
                [Path(p) for p in mod_list]
            ))
    ...
    # ─── Mode --extract: extract TIM2 → image ───────────────────────────────
    if args.extract:
    ...
    # ─── Default mode: convert files from command line ──────────────────────
    if args.list_formats or (not args.images and not args.diff and not args.list):
        list_formats()
        return

    if not args.format:
        print('ERROR: --format is required.  Choose: 4bit | 8bit | 16bit | 24bit | 32bit\n')
        list_formats()
        sys.exit(1)
